...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        Optional<String> tokenOptional = Optional.ofNullable(
                        request.getHeader(ConstantGeneralMessage.CONSTANT_HEADER_AUTHENTICATION))
                .filter(header -> header.startsWith(ConstantGeneralMessage.BEARER_PREFIX))
                .map(header -> header.substring(ConstantGeneralMessage.BEARER_SUBSTRING));
        ...
        if (tokenOptional.isPresent()) {
            ...
            try {
                ...
                log.warn(ConstantGeneralMessage.ERROR_PROCESS_TOKEN + " {} ", e.getMessage());
                response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            }
        }
